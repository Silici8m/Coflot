class RosManager {
    ...
    subscribeFleet(callback) {
        ...
        this.fleetListenerTopic = new ROSLIB.Topic({
            ros: this.ros,
            name: CONFIG.TOPIC_FLEET,
            messageType: CONFIG.TYPE_FLEET 
        });
        this.fleetListenerTopic.subscribe((msg) => {
            const now = Date.now();
    ...
    publishMission(missionMsg) {
        ...
        const res = this.missionRequestTopic.publish(missionMsg);
        console.log("RosManager: Mission publiée", res);
        console.log(`Mission ${missionMsg.name} publiée sur ${CONFIG.TOPIC_MISSION}.`);
    }
}