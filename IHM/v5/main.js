...
function createWaypointTemplate(id) {
    return `<div class="waypoint-card" id="wp-${id}">
        <div class="wp-row-1">
            ...
            <button class="btn-icon btn-cible" onclick="console.log('Viser ${id}')" title="Sélectionner sur la carte">
                <img src="cible.svg" alt="Cible">
            </button>
            
            <button class="btn-icon btn-save-pt" onclick="savePoint(${id})" title="Enregistrer ce point">
                <img src="save.svg" alt="Save">
            </button>
            
            <button class="btn-icon btn-db-delete" onclick="deleteFromDB(${id})" title="Supprimer de la base">
                <img src="delete.svg" alt="Delete DB">
            </button>

            <button class="btn-icon btn-ui-clear" onclick="removeWaypointUI(${id})" title="Enlever de la liste">
...
function savePoint(id) {
    ...
    if (name) {
        ...
        // Ici viendra la logique JS plus tard
    }
}
