class MapManager {
    ...
    updateFleet(fleetMsg) {
        // Liste des robots actuellement reçus
        const currentRobotIds = new Set();

        if (fleetMsg && fleetMsg.robots) {
            fleetMsg.robots.forEach(robot => {
                ...
                marker.x = robot.pose.position.x;
                marker.y = robot.pose.position.y;
                ...
                if (q) {
                    ...
                    // --- CORRECTION ROTATION ---
                    // Utilisation de l'angle positif à cause du scaleY=-1
        ...
        // --- NETTOYAGE ---
        // On parcourt tous les marqueurs existants
        Object.keys(this.fleetMarkers).forEach(id => {
            // Si le marqueur n'est PAS dans la nouvelle liste (et que ce n'est pas le robot principal manuel)
            if (!currentRobotIds.has(id) && id !== "main_robot") {
                const markerToRemove = this.fleetMarkers[id];
                this.fleetLayer.removeChild(markerToRemove);
    ...
    if(el && this.viewer) {
        // ROS2D gère l'aspect ratio en fonction du scale calculé lors du scaleToDimensions
        this.viewer.resize(el.offsetWidth, el.offsetHeight);
        
        // Recadrage optionnel sur la map si elle existe
    ...
    updateRobot(x, y, q) {
        // Si les coordonnées sont invalides, on ne dessine pas
        if (x === null || y === null || q === null) return;

        // 1. Calcul Orientation
        // ROS (Z-up) Quaternion -> Yaw
        const siny_cosp = 2 * (q.w * q.z + q.x * q.y);
        const cosy_cosp = 1 - 2 * (q.y * q.y + q.z * q.z);
        const yaw = Math.atan2(siny_cosp, cosy_cosp);

        const robotId = "main_robot";
        let marker = this.fleetMarkers[robotId];
        ...
        if (!marker) {
            ...
            shape.name = "body";
            marker.addChild(shape);
            this.drawRobotShape(shape, 'busy'); 
            this.fleetLayer.addChild(marker);
            this.fleetMarkers[robotId] = marker;
        }

        marker.visible = true; // On s'assure qu'il est visible
        marker.x = x;
        marker.y = y;

        // --- CORRECTION ROTATION ---
        // Avec un container parent en scaleY = -1, la rotation visuelle s'inverse.
        // Donc +yaw (positif) devrait donner le bon résultat visuel anti-horaire.
        marker.rotation = yaw * (180.0 / Math.PI);
    ...
    hideRobot() {
        const robotId = "main_robot";
        if (this.fleetMarkers[robotId]) {
            this.fleetMarkers[robotId].visible = false;
            this.viewer.scene.update();
        }
    }
